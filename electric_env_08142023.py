...
class ElectricCarEnv(gym.Env):
    ...
    def reward_handler(self,action):
        ...
        if action[0] == 1:
            reward = -0.0002*(1-w)*2 - 1.8*0.13*action[1]*w*2

        elif action[0] == 2:
            reward = -0.0004*(1-w)*2 - 7.6*0.13*action[1]*w*2

        elif action[0] == 3:
            reward = -0.0006*(1-w)*2 - 60*0.26*action[1]*w*2


        elif action[0] == 4:
            reward = -0.001*(1-w)*2 - 400*0.52*action[1]*w*2
    ...
    def step(self,action):
        ...
        if action[0] == 0: #No charging
            ...
            self.tank_size = self.tank_size - (0.0002/100)*(self.tank_size)
            ...
            self.fullchargneed = self.tank_size-self.energy_capacity
            self.tank_size = self.tank_size - (0.0004/100)*(self.tank_size)
            ...
            self.fullchargneed = self.tank_size-self.energy_capacity
            self.tank_size = self.tank_size - (0.0006/100)*(self.tank_size)
            ...
            self.fullchargneed = self.tank_size-self.energy_capacity
            self.tank_size = self.tank_size - (0.001/100)*(self.tank_size)
